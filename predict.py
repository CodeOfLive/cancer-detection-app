...
def get_model():
    global _model
    if _model is None:
        print("🔄 Model ilk kez yükleniyor (Lazy Load)...")
        _model = tf.keras.models.load_model(str(MODEL_PATH))
        print("✅ Model RAM'e alındı!")
    return _model

def predict_image_patches(image_path, threshold=0.8, max_dimension=400):
    model = get_model()  # Sadece ihtiyaç anında yükle
    start = time.time()
    ...
    # Otomatik küçültme (timeout önleme)
    ...
    for y in range(0, h - PATCH_SIZE + 1, PATCH_SIZE):
        for x in range(0, w - PATCH_SIZE + 1, PATCH_SIZE):
            if time.time() - start > 25: break  # 25sn timeout koruması
